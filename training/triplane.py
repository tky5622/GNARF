...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.renderer = ImportanceRenderer(rendering_kwargs, use_triplane=rendering_kwargs.get('use_triplane', False), )
        ...
        self.backbone = StyleGAN2Backbone(
            ...
            w_dim, 
            # img_resolution=256, 
            img_resolution=backbone_resolution, # panic3d add customizability
            # img_channels=32*3
            img_channels=self.triplane_width*3*(
            1 if 'triplane_depth' not in rendering_kwargs else rendering_kwargs['triplane_depth']
            ), # panic3d add customizability
            cond_mode=cond_mode,             # added by panic3d
    ...
    def mapping(
            ...
            update_emas=False):
        # AWB: No body pose conditioning in the generator GNARF
        c = c[:, 0:25]

        if self.rendering_kwargs['c_gen_conditioning_zero']:
                c = torch.zeros_like(c)

        # # shu's fine-tuning hacks panic3d's hack to generate c
        ...
        return self.backbone.mapping(
            ...
            cond, truncation_psi=truncation_psi, 
            truncation_cutoff=truncation_cutoff, 
            update_emas=update_emas)

    def synthesis(self, ws, c, cond=False,
        ...
        # GANRF added this, panic3d don't do this
        # print(c.shape)
        if c.shape[-1] < 111:
        ...
        # ray sampling code
        # condition of "force_rays" added by panic3d
        ...
        N, M, _ = ray_origins.shape
        if use_cached_backbone and self._last_planes is not None: #self._last_planes added by panic3d
             planes = self._last_planes
        else:
            #  planes = self.backbone.synthesis(ws, cond, update_emas=update_emas, **synthesis_kwargs)
            # this conditions are added by panic3d
            if not return_more:
       ...
       # Reshape output into three 32-channel planes
        planes = planes.view(
            len(planes),
            3, self.triplane_width*self.rendering_kwargs.get('triplane_depth',1), #originally 32
            planes.shape[-2],
            planes.shape[-1],
            )  # changed for yichun's multiplane (shu added)
        # Perform volume rendering 
        # xyz_samples are returned for panic3d usecases
        feature_samples, depth_samples, weights_samples, xyz_samples = self.renderer(
            planes, self.decoder,
            ray_origins, ray_directions,
            self.rendering_kwargs,
            # these are used for panic3d
            triplane_crop=triplane_crop,
            cull_clouds=cull_clouds,
            binarize_clouds=binarize_clouds,
            # these are used for GANRF
            smpl_params=smpl_params,
            warp_grid=warp_grid, 
            camera_params=(cam2world_matrix, intrinsics)
            ) # channels last
    ...
    def sample(
            ...
            truncation_cutoff=None, 
            update_emas=False,
            # assethub added it as panic3d don't return run_model
            ignore_run_model=False, 
            **synthesis_kwargs
            ):
        # GNARF originally use self.rendering_kwargs
    ...
    # this f() and set_force_sigmoid is completely new function added by panic3d
    def f(
            self,
            x,
            truncation_psi=1,
...
class OSGDecoder(torch.nn.Module):
    ...
    # this function is added by panic3d
    def set_force_sigmoid(self, state):
...
######## pasting utils ########
#TODO: refactor these functions move it to separate files
